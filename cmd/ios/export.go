...
func init() {
	...
	exportCmd.Flags().IntVarP(&localPort, "local-port", "l", 2222, "ssh port used for local proxy (if the port is occupied randomly)")
	...
	exportCmd.Flags().StringVar(&host, "host", "", "the ssh host of the iOS device (export through the network)")
	exportCmd.Flags().StringVar(&port, "port", "", "the ssh port of the iOS device (export through the network)")
